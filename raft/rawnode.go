...
func (rn *RawNode) Ready() Ready {
	// Your Code Here (2A).
	hardState := hardState(rn.Raft)
	if isHardStateEqual(hardState, rn.hardState) {
		hardState = pb.HardState{}
	} else {
		rn.hardState = hardState
	}
	softState := softState(rn.Raft)
	if isSoftStateEqual(softState, rn.softState) {
		softState = nil
	} else {
